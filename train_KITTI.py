...
parser.add_argument("--log_dir", default='log_Finall_F/')
...
# def dis_loss(xy_points, kp2d, match):
#     loss = 0
#     for i in range(kp2d.shape[0]):
#         p1 = xy_points[i].clone().to('cpu')
#         p2 = kp2d[i].clone().to('cpu').to(torch.float32)
#         m = match[i].clone().to('cpu')
#         matched_points1 = p1[m[:, 0], :]
#         matched_points2 = p2[m[:, 1], :]
#         euclidean_distances = torch.norm(matched_points1 - matched_points2, dim=1, keepdim=True)
#         loss += euclidean_distances.mean()
#     return loss / len(kp2d)
...
def main():
    ...
    # sequence_range_val = [12]
    # sequence_range_test = [13]
    train_data = KITTIDataset(data_root, sequence_range_train, T_range=inT, Rdiff_range=Rdiff_range, Idiff_range=Idiff_range)
    # val_data = KITTIDataset_Sam(data_root, sequence_range_val)
    # test_data = KITTIDataset_Sam(data_root, sequence_range_test)
    ...
    for epoch in range(num_epochs):
        ...
        torch.save(point_cloud_encoder.state_dict(), model_path + f"Encoder_epoch_{epoch + 1}.t7")
